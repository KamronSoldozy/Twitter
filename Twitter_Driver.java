...
class TJTwitter 
{
   ...
   /******************  Part 3 *******************/
   public void investigate ()throws TwitterException, IOException, FileNotFoundException, InterruptedException
   {
     
      Scanner men = new Scanner(new File("male.txt"));
      Scanner woman = new Scanner(new File("female.txt"));
      
      String tweet = "";
      String twet = "";
      int count = 1; 
      int othercount = 1;
      while(men.hasNextLine()){
         for(int x = 0; x<count; x++){
            if(men.hasNextLine())
               tweet = men.nextLine();
         
         }
         //String tweet = men.nextLine();
         int counter = tweet.indexOf("  ");
         tweet = tweet.substring(0, counter);
         twitter.updateStatus("Any common ancestry? @rudwick2 " + "@" + tweet+"rudwick"+" @rudwick" + tweet + " @" +tweet);
         count++;
      ...
      while(woman.hasNextLine()){
         for(int y = 0; y < othercount; y++){
            if(woman.hasNextLine())
               twet = woman.nextLine();
         }
         int newcounter = twet.indexOf("  ");
         twet = twet.substring(0, newcounter);
         twitter.updateStatus("Any common ancestry? @rudwick2 @"+twet+"rudwick"+" @rudwick" + twet + " @" + tweet);
         System.out.println("Another one.");
         Thread.sleep(60000);
         othercount++;
      }
